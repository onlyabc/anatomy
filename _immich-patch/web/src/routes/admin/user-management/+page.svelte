<script lang="ts">
  ...
  let shouldShowDeviceAssignment = $state(false);
  let selectedUser = $state<UserAdminResponseDto>();
  let selectedUserForDevice = $state<UserAdminResponseDto>();
  ...
  /**
   * 拉取每个用户被授权的设备数量，一次性填充 deviceCountMap
   * - 仅管理员可用的接口；后端已校验
   * - 忽略失败以不阻塞主表渲染
   */
  const refreshDeviceCounts = async () => {
    try {
      ...
      // 静默失败：设备数列显示 0 即可，不打扰用户管理主流程
    }
  };

  const openDeviceAssignment = (targetUser: UserAdminResponseDto) => {
    selectedUserForDevice = targetUser;
    shouldShowDeviceAssignment = true;
  };
      ...
      <table class="my-5 w-full text-left">
        ...
        >
          <tr class="flex w-full place-items-center">
            <th class="w-3/12 sm:w-2/12 text-center text-sm font-medium">{$t('phone')}</th>
            <th class="w-5/12 sm:w-3/12 lg:w-4/12 xl:w-3/12 2xl:w-4/12 text-center text-sm font-medium"
              >{$t('email')}</th
            >
            <th class="hidden sm:block w-3/12 text-center text-sm font-medium">{$t('name')}</th>
            <th class="hidden xl:block w-2/12 2xl:w-2/12 text-center text-sm font-medium">{$t('has_quota')}</th>
            <th class="hidden xl:block w-1/12 2xl:w-1/12 text-center text-sm font-medium">设备数</th>
        ...
        <tbody class="block w-full overflow-y-auto rounded-md border dark:border-immich-dark-gray">
          {#if allUsers}
            {#each allUsers as immichUser, index (immichUser.id)}
              ...
              >
                <td class="w-3/12 sm:w-2/12 text-ellipsis break-all px-2 text-sm">{immichUser.phone}</td>
                <td class="w-5/12 sm:w-3/12 lg:w-4/12 xl:w-3/12 2xl:w-4/12 text-ellipsis break-all px-2 text-sm"
                  >{immichUser.email || '-'}</td
                >
                <td class="hidden sm:block w-3/12 text-ellipsis break-all px-2 text-sm">{immichUser.name}</td>
                <td class="hidden xl:block w-2/12 2xl:w-2/12 text-ellipsis break-all px-2 text-sm">
                ...
                <td class="hidden xl:block w-1/12 2xl:w-1/12 text-ellipsis break-all px-2 text-sm">
                  <div class="container mx-auto flex flex-wrap justify-center">
                    {#if immichUser.isAdmin}
                      <span class="text-[#0592dd]">全部</span>
                    {:else}
                      {deviceCountMap[immichUser.id] ?? 0}
                    {/if}
                  </div>
                </td>
                <td
                  ...
                  {#if !immichUser.deletedAt}
                    ...
                    {#if !immichUser.isAdmin}
                      <IconButton
                        ...
                        icon={mdiMonitor}
                        title="设备权限"
                        onclick={() => openDeviceAssignment(immichUser)}
                        aria-label="设备权限"
                      />
                    {/if}
...
{#if shouldShowDeviceAssignment && selectedUserForDevice}
  <UserDeviceAssignmentDialog
    userId={selectedUserForDevice.id}
    userName={selectedUserForDevice.name}
    onClose={() => (shouldShowDeviceAssignment = false)}
    onSaved={(count) => {
      if (selectedUserForDevice) {
        deviceCountMap = { ...deviceCountMap, [selectedUserForDevice.id]: count };
      }
      shouldShowDeviceAssignment = false;
    }}
  />
{/if}
