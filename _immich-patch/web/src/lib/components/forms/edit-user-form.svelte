<script lang="ts">
  ...
  let isAdmin2 = $state((user as any).isAdmin2 || false);
  ...
  const editUser = async () => {
    try {
      const { id, phone, email, name, storageLabel } = user;
...
<FullScreenModal title={$t('edit_user')} icon={mdiAccountEditOutline} {onClose}>
  <form onsubmit={onSubmit} autocomplete="off" id="edit-user-form">
    <div class="my-4 flex flex-col gap-2">
      <label class="immich-form-label" for="phone">{$t('phone')}</label>
      <input class="immich-form-input" id="phone" name="phone" type="tel" required bind:value={user.phone} />
    </div>
    ...
    <div class="my-4 flex flex-col gap-2">
      <label class="immich-form-label" for="is-admin2">{$t('admin.admin2_role')}</label>
      <div class="flex items-center gap-2">
        <input
          ...
          name="is-admin2"
          type="checkbox"
          bind:checked={isAdmin2}
        />
        <span class="text-sm text-gray-500">{$t('admin.admin2_role_description')}</span>
      </div>
    </div>
  </form>
